<script lang="ts" module>
  import { typedContext } from "$lib/utils/svelte.js";

  const { set, get } = typedContext({
    strokeWidth: "1.4",
    classes: "size-4.5",
  ...
  const withClasses = (classes?: string) =>
    classes ? get("classes") + " " + classes : get("classes");

  export {
    set as setIconContext,
    get as getIconContext,
    folderCollapsed,
    folderExpanded,
    file,
    symlink,
...
{#snippet copy()}
  <svg
    ...
    class={withClasses()}
  >
    <g transform="translate(-204.000000, -931.000000)" fill-rule="evenodd">
      <path
...
{#snippet plusFile()}
  ...
  >
    <g>
      ...
      <line x1="15.5" x2="8.5" y1="12" y2="12" />
      <line x1="12" x2="12" y1="15.5" y2="8.5" />
    </g>
  </svg>
{/snippet}

{#snippet plusFolder()}
  {@const modWidth = Math.max(parseFloat(get("strokeWidth")) / 1.6, 0)}
  <svg
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    stroke-width={modWidth}
    stroke="currentColor"
    class={withClasses()}
  >
    <g stroke-width="0" />
